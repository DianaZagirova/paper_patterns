...
This script collects data on papers from dois_minus_list.py that mimic aging theory papers
but are not actually about aging theories. It saves the results to a separate directory.
"""
import os
import time
import json
...
DATA_DIR = './data/negative_examples'
...
def get_paper_data_from_doi(doi: str) -> Optional[Dict[str, Any]]:
    ...
    paper_data = {
        ...
        'is_negative_example': True,  # Mark as negative example
    }
...
def main():
    ...
    if 'dois' not in globals() or dois is None:
        try:
            ...
            from dois_minus_list import dois
        except ImportError:
            log_message("Error: Could not import DOI list. Make sure dois_minus_list.py exists or use the wrapper script.")
            return
...
if __name__ == "__main__":
    try:
        ...
        log_message("Starting negative examples data collection process")
        main()
