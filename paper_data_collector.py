...
# NCBI API rate limiting (max 10 requests/sec, we use 3 to be safe)
...
def safe_api_call(func, *args, **kwargs):
    ...
    for attempt in range(tries):
        try:
            ...
            if attempt < tries-1:
                log_message(f"Retrying {func.__name__} for args {args} (Error: {str(e)})")
                time.sleep(2)
            else:
                log_message(f"Failed {func.__name__} for args {args}, skipping (Error: {str(e)})")
                return None
